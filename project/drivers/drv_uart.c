...
{	
  ...
  UARTConfigSetExpClk(uart->hw_base, SysCtlClockGet(), cfg->baud_rate, config);
	UARTFIFOEnable(uart->hw_base);

	// Enable the UART.
	UARTEnable(uart->hw_base);
    return RT_EOK;
}
...
static rt_err_t hw_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CLR_INT:
        /* disable rx irq */
         UARTIntDisable(uart->hw_base, UART_INT_RX | UART_INT_RT);
        break;
    case RT_DEVICE_CTRL_SET_INT:
        /* enable rx irq */
         UARTIntEnable(uart->hw_base, UART_INT_RX | UART_INT_RT);
        break;
    }
...
static int hw_putc(struct rt_serial_device *serial, char c)
{
    ...
    uart = mUartGetHwPtr(serial);
     ...
     UARTCharPut(uart->hw_base, *((uint8_t *)&c));
    return 1;
}
...
static int hw_getc(struct rt_serial_device *serial)
{
	...
	return  UARTCharGetNonBlocking(uart->hw_base);
}
...
void UART0_IRQHandler(void)
{
    ...
    {
         UARTIntClear(uart->hw_base, intsrc);
