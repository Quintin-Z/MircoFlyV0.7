...
#include <stdint.h>
#include <stdbool.h>
#include "driverlib/rom.h"//by zjq
...
void getclock(void)
{
	rt_kprintf("The device is runing on %dMHz.\n",ROM_SysCtlClockGet()/1000000);

}
FINSH_FUNCTION_EXPORT(getclock,get device runing clock.)

void reboot(void)
{
	ROM_SysCtlReset();
}
FINSH_FUNCTION_EXPORT(reboot,reboot device now. )
